...
public extension PrimitiveSequence where Trait == SingleTrait {
    ...
    /**
     ...
     - returns: The source sequence with the side-effecting behavior applied.
     */
    func doOnSuccess<A: AnyObject>(weak obj: A, _ onSuccess: @escaping (A) -> () throws -> Void) -> Single<Element> {
        return self.do(onSuccess: { [weak obj] _ in
            guard let obj = obj else { return }
            return try onSuccess(obj)()
        })
    }
}
...
public extension PrimitiveSequence where Trait == SingleTrait {
    ...
    /**
     ...
     - returns: The source sequence with the side-effecting behavior applied.
     */
    func doOnError<A: AnyObject>(weak obj: A, _ onError: @escaping (A) -> () throws -> Void) -> Single<Element> {
        return self.do(onError: { [weak obj] _ in
            guard let obj = obj else { return }
